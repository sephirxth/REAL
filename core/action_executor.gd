## R.E.A.L. Action Executor (C4) - Universal Version
## Execute LLM-generated debug commands (UDP + Web JavaScript bridge)
##
## Usage:
##   1. Add as autoload: Project > Project Settings > Autoload > Add "ActionExecutor"
##   2. Register entities: ActionExecutor.register_entity("Player", node, state_dict)
...
const DEFAULT_UDP_PORT := 19999
const RESULT_FILE := "user://commands/result.yaml"
const WorldConfig = preload("res://data/world_config.gd")
...
var _udp_port: int = DEFAULT_UDP_PORT
var _logger: Node
...
func _ready() -> void:
	process_mode = Node.PROCESS_MODE_ALWAYS  # Keep C4 responsive during pause
	...
	if _is_check_only_run():
		print("[C4] Check-only mode - skipping runtime initialization")
		call_deferred("_quit_check_only_run")
		return

	_udp_port = _resolve_udp_port()
	...
	if _is_web:
		...
		print("[C4] Listening on UDP port %d" % _udp_port)


func _resolve_udp_port() -> int:
	for key in ["C4_UDP_PORT", "C4_PORT"]:
...
func _setup_udp() -> void:
	...
	var err := _udp.bind(_udp_port)
	if err != OK:
		push_error("[C4] Failed to bind UDP port %d: %s" % [_udp_port, err])
...
func _setup_js_bridge() -> void:
	...
	window.Game = {
		...
		help: function() {
			console.log('R.E.A.L. C4 Commands:');
			console.log('  Game.get("Eileen", "cognition")   - Read entity state');
	...
	console.log('[R.E.A.L.] C4 ready. Type Game.help() for commands.');
	"""
	JavaScriptBridge.eval(js_code)
	print("[C4] JavaScript bridge initialized - use Game.help() in browser console")
...
func _poll_js_commands() -> void:
	...
	if result != null and result is String:
		...
		if err == OK:
			...
			print("[C4] Received command via JavaScript")
...
func execute(command: Dictionary) -> Dictionary:
	...
	print("[C4] Execute: %s -> %s" % [cmd_type, "OK" if result.success else result.message])
...
func _process_yaml(content: String, sender_ip: String = "", sender_port: int = 0) -> void:
	...
	print("[C4] Received command via UDP from %s:%d" % [sender_ip, sender_port])
...
func _cmd_set(cmd: Dictionary) -> Dictionary:
	...
	if _logger: _logger.log_event("C4.set", {"entity": entity_id, "field": field, "old": old_value, "new": value})
...
func _try_set_live_entity_field(entity_id: String, node: Node, field: String, value) -> Dictionary:
	if node == null or field.is_empty():
		return {}
	if field == "cognition":
		if not node.has_method("apply_cognition_state"):
			return {}
		if not (value is Dictionary):
			return {"success": false, "message": "Set %s.%s requires a Dictionary value" % [entity_id, field]}
		var old_value = node.call("get_cognition_state") if node.has_method("get_cognition_state") else {}
		var applied := bool(node.call("apply_cognition_state", value))
		if not applied:
			return {"success": false, "message": "Failed to apply %s.%s" % [entity_id, field]}
		var new_value = node.call("get_cognition_state") if node.has_method("get_cognition_state") else value
		if _state_refs.has(entity_id):
			_state_refs[entity_id]["cognition"] = new_value
		if _logger:
			_logger.log_event("C4.set", {"entity": entity_id, "field": field, "old": old_value, "new": new_value})
		return {"success": true, "message": "Set %s.%s" % [entity_id, field], "value": new_value}
	if field in ["arousal", "valence", "free_energy", "surprise"]:
		if not node.has_method("prime_affective_state"):
			return {}
		var old_value = null
		var new_value = value
		if node.has_method("get_cognition_state"):
			var old_state_value = node.call("get_cognition_state")
			if old_state_value is Dictionary:
				old_value = (old_state_value as Dictionary).get(field, null)
		node.call("prime_affective_state", {field: value}, "c4_set")
		if node.has_method("get_cognition_state"):
			var new_state_value = node.call("get_cognition_state")
			if new_state_value is Dictionary:
				var new_state := (new_state_value as Dictionary)
				new_value = new_state.get(field, value)
				if _state_refs.has(entity_id):
					_state_refs[entity_id]["cognition"] = new_state.duplicate(true)
		if _state_refs.has(entity_id):
...
func _cmd_get(cmd: Dictionary) -> Dictionary:
	...
	var found := false
	var node: Node = _entity_refs.get(entity_id, null)
	if field == "cognition" and node and node.has_method("get_cognition_state"):
		value = node.get_cognition_state()
		found = true
	elif field == "partner_id" and node and node.has_method("get_cooperation_partner_id"):
		value = node.call("get_cooperation_partner_id")
		found = true
	elif field == "cooperation_score" and node and node.has_method("get_cooperation_debug"):
		var cooperation_debug_value = node.call("get_cooperation_debug")
		if cooperation_debug_value is Dictionary:
			value = float((cooperation_debug_value as Dictionary).get("cooperation_score", 0.0))
			found = true
	elif field == "cooperation" and node and node.has_method("get_cooperation_debug"):
		value = node.call("get_cooperation_debug")
		found = true
	...
	if _logger:
		_logger.log_event("C4.get", {"entity": entity_id, "field": field, "value": value_text})
	return {
		"success": true,
...
func _cmd_teleport(cmd: Dictionary) -> Dictionary:
	...
	if dimension_mode == "2d":
		...
		if node is Node2D:
			...
			if _logger: _logger.log_event("C4.teleport", {"entity": entity_id, "from": old_pos, "to": new_pos})
...
func _cmd_spawn(cmd: Dictionary) -> Dictionary:
	...
	if _logger:
		_logger.register_entity(entity_id, state)
		_logger.log_event("C4.spawn", {"id": entity_id, "prefab": prefab_name, "position": pos})
...
func _cmd_destroy(cmd: Dictionary) -> Dictionary:
	...
	var node: Node = _entity_refs[entity_id]
	if _logger: _logger.log_event("C4.destroy", {"entity": entity_id})
	node.queue_free()
...
func _cmd_pause(_cmd: Dictionary) -> Dictionary:
	...
	if screen_manager and screen_manager.has_method("pause_game"):
		if not bool(screen_manager.get("is_paused")):
			screen_manager.pause_game()
	else:
		get_tree().paused = true
	if _logger: _logger.log_event("C4.pause", {})
	return {"success": true, "message": "Game paused"}


func _cmd_resume(_cmd: Dictionary) -> Dictionary:
	var screen_manager := _get_screen_manager()
	if screen_manager and screen_manager.has_method("resume_game"):
		if bool(screen_manager.get("is_paused")):
			screen_manager.resume_game()
	else:
		get_tree().paused = false
	if _logger: _logger.log_event("C4.resume", {})
...
func _cmd_timescale(cmd: Dictionary) -> Dictionary:
	...
	# Keep engine timing stable for rendering/physics, and accelerate the game's day/night loop
	# through the config knob that the project already uses in runtime settings.
	Engine.time_scale = 1.0
	WorldConfig.game_speed = scale
	if _logger:
		_logger.log_event("C4.timescale", {
			"requested_scale": requested_scale,
			"scale": scale,
			"engine_time_scale": Engine.time_scale,
			"game_speed": WorldConfig.game_speed,
		})
	return {"success": true, "message": "Time scale set to %s" % scale}


func _cmd_capture(cmd: Dictionary) -> Dictionary:
	var reason: String = cmd.get("reason", "C4_TRIGGER")
	...
	if _logger:
		_logger.capture(reason, capture_context)
...
func _cmd_debug_collision(cmd: Dictionary) -> Dictionary:
	...
	if _logger: _logger.log_event("C4.debug_collision", {"enabled": enabled})
...
func _cmd_persist(cmd: Dictionary) -> Dictionary:
	...
	if _logger:
		_logger.log_event("C4.persist", {"count": spawns.size(), "output": output_path})
...
func _cmd_edit_mode(cmd: Dictionary) -> Dictionary:
	...
	if _logger:
		_logger.log_event("C4.edit_mode", {"enabled": enabled, "level": level_name})
...
func _cmd_export_layout(cmd: Dictionary) -> Dictionary:
	...
	if _logger and result.success:
		_logger.log_event("C4.export_layout", {
			"level": level_name,
...
func _generate_level_code(spawns: Array[Dictionary]) -> String:
	...
	lines.append("## Generated by R.E.A.L. C4 persist command")
...
func _get_screen_manager() -> Node:
	...
	if not tree or not tree.current_scene:
		return null
	var screen_manager = tree.current_scene.get("_screen_manager")
	if screen_manager is Node and is_instance_valid(screen_manager):
...
func _show_collision_visualizers() -> void:
	...
	print("[C4] Collision visualizers created: %d" % _collision_visualizers.size())
...
func _send_udp_response(ip: String, port: int, results: Array[Dictionary]) -> void:
	...
	if _udp != null:
		...
		_udp.put_packet(response.to_utf8_buffer())
...
func _write_results(results: Array[Dictionary]) -> void:
	...
	var lines: PackedStringArray = []
	lines.append("# C4 Execution Results")
...
# ============================================
# Input Simulation Commands (C4.5)
# ============================================
...
func _dispatch_viewport_input(event: InputEvent) -> void:
	...
	if viewport and event is InputEventMouse:

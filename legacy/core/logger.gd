## R.E.A.L. Logger (C1) - Universal Version
## Debug log system based on axiomatic derivation
## Output format adapts to Claude Code's Glob/Grep/Read capabilities
##
## Usage:
##   1. Add as autoload: Project > Project Settings > Autoload > Add "Logger"
##   2. Call Logger.set_tick(tick) each frame
##   3. Call Logger.log_event("EventType", {data}) for events
...
var _visual_grounder: Node = null
...
func _init_session() -> void:
	...
	print("[Logger] Session: %s" % global_session)
...
func _create_latest_symlink(log_dir: String, session_path: String) -> void:
	...
	if not OS.has_feature("web") and not OS.has_feature("windows"):
		...
		if err == OK:
			print("[Logger] Symlink: %s -> latest" % session_path.get_file())
...
func _setup_web_log_viewer() -> void:
	...
	console.log('[R.E.A.L.] C1 Logger ready. Use GameLogs.showHistory() or GameLogs.showCaptures()');
	"""
...
## Set C3 Visual Grounder reference
func set_visual_grounder(grounder: Node) -> void:
	_visual_grounder = grounder
...
func capture(trigger_summary: String, context: Dictionary = {}) -> String:
	...
	if _enable_screenshots and _visual_grounder and _visual_grounder.has_method("capture_screenshot"):
		screenshot_path = await _visual_grounder.capture_screenshot(_session_dir, summary, _current_tick)
		if _visual_grounder.has_method("get_annotation_map"):
			annotations = _visual_grounder.get_annotation_map()
...
func _resolve_run_id(fallback: String) -> String:
	for key in ["REAL_RUN_ID", "RUN_ID", "GODOT_PROCESS_TAG"]:
...
func _init_observability_storage() -> void:
	...
	if not FileAccess.file_exists(_timeline_path):
		...
		if timeline_file:
			...
			push_error("[Logger] Failed to initialize timeline: %s" % _timeline_path)
...
func _write_capture_file(filepath: String, content: String) -> void:
	...
	if file:
		...
		push_error("[Logger] Failed to write: %s" % filepath)
...
func _write_json_file(path: String, payload: Dictionary) -> bool:
	...
	if not file:
		push_error("[Logger] Failed to write json: %s" % path)
		return false
...
func _append_jsonl(path: String, payload: Dictionary) -> void:
	...
	if not file:
		push_error("[Logger] Failed to append jsonl: %s" % path)
		return
	file.seek_end()
